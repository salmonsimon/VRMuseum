...
class OCULUSINTERACTION_API IIsdkIRootPose
{
 ...
 public:
  ...
  /// Returns conditional of the state of whether or not the device associated with this root pose
  /// is enabled in the system, and enumerated in the list of connected devices.
  UFUNCTION(BlueprintNativeEvent, BlueprintCallable, Category = InteractionSDK)
  ...
  /// Returns conditional of the state of if the device associated with this root pose is connected,
  /// the root pose is valid and was set from tracking data is marked as high confidence. If the
  /// conditional is true, then it implies that IsRootPoseConnected and IsRootPoseValid are also
  /// true, so they don't need to be checked in addition to this. Conditional is false if
  /// IsRootPoseConnected or IsRootPoseValid is false.
  UFUNCTION(BlueprintNativeEvent, BlueprintCallable, Category = InteractionSDK)
